...
const Cart = (props) => {
    ...
    const onAddOverlay = async (obj)=>{
        try{
            ...
            if(findOverlay)
            {
            axios.delete(`https://639709ed77359127a02ac092.mockapi.io/cart/${findOverlay.id}`)
            props.setOverlayItems((over) => over.filter(item => item.myId !== obj.myId))
            }
            else{
              const {data} = await axios.post('https://639709ed77359127a02ac092.mockapi.io/cart', obj)
